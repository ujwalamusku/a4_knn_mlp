...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        self._y = np.transpose(one_hot_encoding(y))

        np.random.seed(42)

        self._h_weights = np.random.uniform(0, 1, (self.n_hidden, self._X.shape[0]))
        self._o_weights = np.random.uniform(0, 1, (self._y.shape[0], self.n_hidden))
        self._h_bias = np.ones((self.n_hidden, 1))
        self._o_bias = np.ones((self._y.shape[0], 1))
    ...
    def fit(self, X, y):
        ...
        history_list = np.array([])
        #print("activation function is ", str(self.hidden_activation))
        for iter in range(self.n_iterations):

            #print("there",self._y.shape)
            #Feed forward
            in_to_hid = np.dot(self._h_weights, self._X) + self._h_bias
            str1 = "(" + "in_to_hid" + ")"
            initial_pred = self.hidden_activation(eval(str1))

            hid_to_out = np.dot(self._o_weights, initial_pred) + self._o_bias
            str1 = "(" + "hid_to_out" + ")"
            final_pred = softmax(eval(str1))

            #Backward
            str1 = "hid_to_out"
            diff = (final_pred - self._y)
            delta_out = (final_pred - self._y) * softmax(eval(str1), derivative=True)
            final_delta_out = np.dot(delta_out, np.transpose(initial_pred))

            str1 = "in_to_hid"
            #delta_hid = np.dot(np.transpose(self._o_weights), delta_out) * self.hidden_activation(eval(str1),derivative = True)
            delta_hid = np.dot(np.transpose(self._o_weights), diff) * self.hidden_activation(eval(str1),derivative = True)
            final_delta_hid = np.dot(delta_hid, np.transpose(self._X))

            #Update weights and bias
            self._h_weights -= self.learning_rate * final_delta_hid
            self._o_weights -= self.learning_rate * final_delta_out
            self._h_bias -= self.learning_rate * np.transpose([delta_hid.sum(axis=1)])
            self._o_bias -= self.learning_rate * np.transpose([delta_out.sum(axis=1)])

            history_list = np.append(history_list,cross_entropy(self._y,final_pred))
    ...
    def predict(self, X):
        ...
        # Feed forward
        test_in_to_hid = np.dot(self._h_weights, X) + self._h_bias
        str1 = "test_in_to_hid"
        test_initial_pred = self.hidden_activation(eval(str1))

        test_hid_to_out = np.dot(self._o_weights, test_initial_pred) + self._o_bias
        str1 = "test_hid_to_out"
        #test_final_pred = self.hidden_activation(eval(str1))
        test_final_pred = softmax(eval(str1))

        test_final_pred = np.transpose(test_final_pred)
        result = test_final_pred.argmax(axis=1)
